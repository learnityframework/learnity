...
extends JSPGrid {
    ...
    private int aj = -1;
    private int at = -1;
    private String[] col_names = null;
    private String[][] col_data = null;
    ...
    @Override
    public int getRows() {
        if (this.col_names != null && this.col_data != null) {
            if (this.ap) {
                return this.rowno;
            }
            return 0;
    ...
    public JSPGridPro1(int i1, int j1, HttpServletRequest request, String s) {
        super.initGrid(i1, j1, request, s);
        this.initGrid(request, s);
    }

    public JSPGridPro1(int i1, int j1) {
        super.initGrid(i1, j1);
    }

    public JSPGridPro1(HttpServletRequest request, String s) {
        this.initGrid(request, s);
    }
    ...
    public void setConnectionParameters(String[] s, String[][] s1) {
        this.col_names = s;
        this.col_data = s1;
        this.makeConnection();
    }
    ...
    private void makeConnection() {
        this.numberOfColumns = this.col_names.length;
        super.initGrid(this.numberOfColumns, 0);
    }

    private void getResultSet() {
        try {
            this.rowno = this.col_data.length;
            if (this.W == -1) {
    ...
    private void processResultSet() {
        ...
        boolean flag = false;
        try {
            System.out.println("W = " + this.W + " as = " + this.as);
            while (this.W + i1 - 1 != this.rowno) {
                super.addRow();
                for (int j1 = 1; j1 <= this.numberOfColumns; ++j1) {
                    this.a(j1, this.col_names[j1 - 1]);
                    if (!super.Cols(j1 - 1).getFieldType().equals("IMAGE")) {
                        super.Cols(j1 - 1).Cells(i1).setFieldValue(this.col_data[this.W + i1 - 1][j1 - 1]);
                        continue;
                    }
                    super.Cols(j1 - 1).Cells(i1).setImageSource(this.col_data[this.W + i1 - 1][j1 - 1]);
                }
                if (++i1 < this.as) continue;
            }
            super._mthtry(i1);
        }
        catch (Exception exp) {
    ...
    private int h() {
        int i1 = 0;
        try {
            if (this.req.getParameter("__gridpro1") == null || this.req.getParameter("__gridpro1").equals("1") || this.au) {
                i1 = 1;
                this.ag = 1;
            } else {
    ...
    public String getPageNavigationHTML() {
        ...
        for (int k1 = 0; k1 < this.H && j1 + k1 <= i1; ++k1) {
            s = j1 + k1 != this.ag ? s + " " + "<a href=\"javascript:document." + this.N + ".__action.value='page';" + "document." + this.N + ".__gridpro1.value='" + (j1 + k1) + "';" + "document." + this.N + ".action='" + this.c() + "';" + "document." + this.N + ".submit();\">" + (j1 + k1) + "</a>" : s + " " + (j1 + k1);
        }
        if (i1 != this.ag) {
            s = s + " <a href=\"javascript:document." + this.N + ".__action.value='page';" + "document." + this.N + ".__gridpro1.value='" + (this.ag + 1) + "';" + "document." + this.N + ".action='" + this.c() + "';" + "document." + this.N + ".submit();\">" + this.af + "</a>" + this.l();
        }
        if (1 != this.ag) {
            s = this.n() + " <a href=\"javascript:document." + this.N + ".__action.value='page';" + "document." + this.N + ".__gridpro1.value='" + (this.ag - 1) + "';" + "document." + this.N + ".action='" + this.c() + "';" + "document." + this.N + ".submit();\">" + this.aa + "</a> " + s;
        }
    ...
    private String e() {
        return "<INPUT TYPE=\"HIDDEN\" NAME=\"__gridpro1\" VALUE=\"1\"><INPUT TYPE=\"HIDDEN\" NAME=\"__gridpro2\" VALUE=\"" + this.aj + "," + this.ab + "," + this.at + "\">";
    }
    ...
    private String n() {
        String s = "";
        if (this.ac) {
            s = " <a href=\"javascript:document." + this.N + ".__action.value='page';" + "document." + this.N + ".__gridpro1.value='1';" + "document." + this.N + ".action='" + this.c() + "';" + "document." + this.N + ".submit();\">" + this.K + "</a>";
        }
        return s;
    }
    ...
    private String l() {
        String s = "";
        if (this.X) {
            s = " <a href=\"javascript:document." + this.N + ".__action.value='page';" + "document." + this.N + ".__gridpro1.value='-1';" + "document." + this.N + ".action='" + this.c() + "';" + "document." + this.N + ".submit();\">" + this.R + "</a>";
        }
        return s;
    }
