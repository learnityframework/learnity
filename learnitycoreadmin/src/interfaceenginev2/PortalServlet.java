...
/**
 * Title: DisplayEngine Description: Portal Opening Page Copyright: Copyright
 * (c) 2008 Company: Aunwesha Knowledge Technologies Pvt. Ltd.
 * 
...
public class PortalServlet extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		System.out.println("=============useInterfaceCaching======"
				+ useInterfaceCaching);
		...
		vApplicationTemplate = (Vector) sc
				.getAttribute("ApplicationTemplateConf");
		vDefaultApplicationTemplate = (Vector) sc
				.getAttribute("DefaultApplicationTemplateConf");
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (getRoleCheck == null)
			getRoleCheck = "";
		String getContentTypeHtml = NewDataBaseLayer
				.getContentTypeHtml(InterfaceID);
		String cacheStatus = "";
		...
		if (getRoleCheck.equals("") || getRoleCheck.equals("true")) {
			...
			if (user_id == null) {
				...
				String page = ICP.getInterfaceFromCacheName(cache_name,
						cache_key);
				if (page == null)
					page = "";
				...
				if (page.equals("")) {
					...
					System.out.println("=========caching_status=======11111==="
							+ caching_status);
					if (caching_status == true) {
						String role_id = NewDataBaseLayer.getRoleID(user_id);
						String interfaceengine_role_id = NewDataBaseLayer
								.getInterfaceengineRoleID(role_id.toUpperCase());
						System.out
								.println(".................interfaceengine_role_id........"
										+ interfaceengine_role_id);
						Vector imagefile = NewDataBaseLayer.getHTMLAsString(
								interfaceengine_role_id, InterfaceID);
						if (imagefile != null) {
							for (int j = 0; j < imagefile.size(); j = j + 1) {
								// in = (InputStream)imagefile.elementAt(0);
								// String page_string =
								// org.apache.commons.io.IOUtils.toString(in);
								String page_string = (String) imagefile
										.elementAt(0);
								ICP.setInterfaceFromCacheName(cache_name,
										cache_key, page_string);
								in = org.apache.commons.io.IOUtils
										.toInputStream(page_string);
								int len = 0;
						...
						System.out
								.println(".................interfaceengine_role_id........"
										+ interfaceengine_role_id);
						Vector imagefile = NewDataBaseLayer.getHTML(
								interfaceengine_role_id, InterfaceID);
						if (imagefile != null) {
							for (int j = 0; j < imagefile.size(); j = j + 1) {
								in = (InputStream) imagefile.elementAt(0);
			...
			if (page.equals("")) {
				boolean caching_status = checkCachingRequired(InterfaceID);
				System.out.println("=========caching_status=======22222==="
						+ caching_status);
				if (caching_status == true) {
					...
					System.out
							.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>role_id>>>>>>>>>>"
									+ role);
					Vector imagefile = NewDataBaseLayer.getHTML(role_id,
							InterfaceID);
					if (imagefile != null) {
						for (int j = 0; j < imagefile.size(); j = j + 1) {
							in = (InputStream) imagefile.elementAt(0);
			...
			if (page.equals("")) {
				boolean caching_status = checkCachingRequired(InterfaceID);
				System.out.println("=========caching_status=======3333==="
						+ caching_status);
				if (caching_status == true) {
					String role = NewDataBaseLayer.getDefaultRoleID(d);
					Vector imagefile = NewDataBaseLayer.getHTMLAsString(role,
							InterfaceID);
					if (imagefile != null) {
						for (int j = 0; j < imagefile.size(); j = j + 1) {
							String page_string = (String) imagefile
					...
					System.out
							.println("==========checkCachingRequired====false========");
					String role = NewDataBaseLayer.getDefaultRoleID(d);
					Vector imagefile = NewDataBaseLayer.getHTML(role,
							InterfaceID);
					if (imagefile != null) {
						for (int j = 0; j < imagefile.size(); j = j + 1) {
							in = (InputStream) imagefile.elementAt(0);
	...
	public boolean checkCachingRequired(String InterfaceID) {
		...
		if (useInterfaceCaching.equals("true")
				&& cacheStatus.equals("STATUS_ALIVE")) {

			String InterfaceCachingStatus = NewDataBaseLayer
					.getInterfaceCachingStatus(InterfaceID);
			System.out.println("================InterfaceCachingStatus======"
					+ InterfaceCachingStatus);
			...
			if (InterfaceCachingStatus.equalsIgnoreCase("true")
				...
				if (InterfaceCachingStatus.equalsIgnoreCase("false"))
					...
					String application_template_id = NewDataBaseLayer
							.getApplicationTemplateId(InterfaceID);
					String template_caching_status = "";
					System.out
							.println("===============application_template_id======"
									+ application_template_id);
					if (vApplicationTemplate != null) {
						for (int i = 0; i < vApplicationTemplate.size(); i++) {
							Vector vApplicationTemplateSub = (Vector) vApplicationTemplate
									.elementAt(i);
							String v_template_id = (String) vApplicationTemplateSub
									.elementAt(0);
							if (v_template_id.equals(application_template_id)) {
								template_caching_status = (String) vApplicationTemplateSub
										.elementAt(2);
							}
						}
					}
					if (template_caching_status.equals("true"))
						...
						if (vDefaultApplicationTemplate != null) {
							for (int i = 0; i < vDefaultApplicationTemplate
									.size(); i++) {
								Vector vDefaultApplicationTemplateSub = (Vector) vDefaultApplicationTemplate
										.elementAt(i);
								d_template_caching_status = (String) vDefaultApplicationTemplateSub
										.elementAt(2);
				...
				if (vApplicationTemplate != null) {
					for (int i = 0; i < vApplicationTemplate.size(); i++) {
						...
						String v_template_id = (String) vApplicationTemplateSub
								.elementAt(0);
						if (v_template_id.equals(application_template_id)) {
							cache_name = (String) vApplicationTemplateSub
									.elementAt(3);
						}
					}
				}
				if (cache_name.equals("")) {
					if (vDefaultApplicationTemplate != null) {
						for (int i = 0; i < vDefaultApplicationTemplate.size(); i++) {
							Vector vDefaultApplicationTemplateSub = (Vector) vDefaultApplicationTemplate
									.elementAt(i);
							cache_name = (String) vDefaultApplicationTemplateSub
									.elementAt(3);

						}
					}
					System.out.println("======222====cache_name======="
							+ cache_name);
					if (cache_name.equals("")) {
