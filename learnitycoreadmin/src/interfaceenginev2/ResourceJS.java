...
public boolean checkCachingRequired(String InterfaceID)
{
	...
	if(useInterfaceCaching.equals("true") && cacheStatus.equals("STATUS_ALIVE"))
	{
		...
		String ResourceJSCachingStatus = NewDataBaseLayer.getResourceJSCachingStatus(InterfaceID);
		System.out.println("================ResourceJSCachingStatus======"+ResourceJSCachingStatus);
		cache_name = NewDataBaseLayer.getInterfaceCacheName(InterfaceID);
		System.out.println("==========cache_name======="+cache_name);
		
		if(ResourceJSCachingStatus.equalsIgnoreCase("true") && !cache_name.equals(""))
		{
			flag = true;
		}//end of InterfaceCachingStatus true if
		else
		{
			if(ResourceJSCachingStatus.equalsIgnoreCase("false"))
				flag = false;
			else
			{
				String application_template_id = NewDataBaseLayer.getApplicationTemplateId(InterfaceID);
				String template_js_caching_status = "";
				...
				if(vApplicationTemplate!=null)
				{
					for(int i=0;i<vApplicationTemplate.size();i++)
					{
						...
						{
							template_js_caching_status = (String)vApplicationTemplateSub.elementAt(4);
						}
					}
				}
				if(template_js_caching_status.equals("true"))
					flag = true;
				else
				{
					String d_template_js_caching_status = "";
					System.out.println("===============application_template_id======"+application_template_id);
					if(vDefaultApplicationTemplate!=null)
					{
						for(int i=0;i<vDefaultApplicationTemplate.size();i++)
						{
							...
							d_template_js_caching_status = (String)vDefaultApplicationTemplateSub.elementAt(4);
							
						}
					}
					if(d_template_js_caching_status.equals("true"))
						flag = true;
				}
			}
